...
public class AccountActivationSuccessEventPublisher {
    ...
    public void publishAccountActivationSuccessEvent(AccountActivationSuccessEvent event) {
        Map<String, String> message = Map.of("email", event.getEmail());
        MapRecord<String, String, String> rec = 
            StreamRecords.newRecord().ofMap(message).withStreamKey("account-activation-success-event");
